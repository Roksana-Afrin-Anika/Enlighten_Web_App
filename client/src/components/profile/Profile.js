...
const ProfileForm = () => {
  ...
  // Populate form data with fetched profile data and the logged-in user's name
  useEffect(() => {
    if (profile) {
      setFormData((prev) => ({
        ...prev,
        ...
        nativeLanguage: profile.nativeLanguage || "English (English)",
        fluentLanguage: profile.fluentLanguage || "French (Français)",
        learningLanguage:
          profile.learningLanguage || "Chinese (Traditional) (中文 (繁體))",
